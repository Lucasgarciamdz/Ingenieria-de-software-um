...
export class UsuarioService {
    ...
    obtenerMensajesPorEtiqueta(nombre: string) {
        console.log(" --- Buscando mensajes de etiquetas: ", nombre)
        const url = `${environment.url}/mensajes/etiqueta/` + nombre;
        console.log("URL", url)
        return new Promise<any | void>((resolve, reject) => {
            this.http.get<BaseResponse>(url, this.authSvc.headers_http).subscribe(
                {
    ...
    editarMensaje(mensajeEditado: any, idMensaje : number) {
        console.log(" --- Editando mensaje de id: ", idMensaje)
        const url = `${environment.url}/mensajes/` + idMensaje;
        console.log("URL", url)
        return new Promise<any | void>((resolve, reject) => {
            this.http.put<BaseResponse>(url,mensajeEditado, this.authSvc.headers_http).subscribe(
                {
                    next(res: any) {
                        console.log("---- Se hizo la petición : ", res)
                        resolve(res)
