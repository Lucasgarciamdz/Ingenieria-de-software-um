...
public interface UsuarioRepository extends BaseRepository<Usuario, Long> {
    ...
    @Query(value="SELECT s FROM seguidores_seguidos s WHERE s.seguido_id = :idUsuario", nativeQuery = true)
    List<Usuario> findAllFollowers(@Param("idUsuario") Long idUsuario);

    @Query(value="SELECT s FROM seguidores_seguidos s WHERE s.seguidor_id = :idUsuario", nativeQuery = true)
